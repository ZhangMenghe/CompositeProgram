...
void setupDebugRoom(Room* room){
    ...
    float objParam[] = {0, 0, 40, 40, 0, 0, 10};
    float bedParam[] = {0, 0, 180, 200, 0, 4, 10};
...
string getFixedRoomAndObjs(string input) {
	return "r : 400,300\nw : -200, 150, 200, 150\nw : 200, -150, 200, 150\nw : -200, -150, 200, -150\nw: -200, -150, -200, 150\nf : 0, 0, 100, 50, 10, 0, 0";
}
void parser_customer_input_string(string rawString, Room * room, bool needFixedRoomAndObjs) {
	if (!rawString.length())
		return;
	if (needFixedRoomAndObjs)
		rawString = getFixedRoomAndObjs(rawString);
    float mWeights[] = {1.0f, 0.01f, 0.01f, 0.1f, 1.0f, 1.0f, 1.0f, 0.001f, 0.01f, 1.0f, 2.0f};
